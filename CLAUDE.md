...
3. **イラストは「ひつじ村」風のドット絵**（2026-09-20 に SVG ベクター絵から変更。ユーザーの要望）：**カメラは 3/4 俯瞰で統一**（2026-09-21 に刷新）。背景の牧場が見下ろし視点なので、ハムスターもグッズも「斜め上から見た形」で描く。正面立ち絵（顔と胸が正対する猫のような二足の座り姿）は**パースが合わず却下済み**。ハムスターは**背中が見える丸い塊（饅頭）**で、顔は塊の下半分、耳は上（背中側にかぶさる）、手はあごの下、足は塊の左右下から少し。顔の記号はねこあつめ流：縦 1×3 の点目、鼻を頂点にした小さな ω、ほっぺの外に突き出るヒゲ、おなか色のマズル。グッズは屋根・天面・水面が見える台形／楕円で描く。1px の細い暗い輪郭＋ベタ塗り、影・グラデなし。
   ...
   - 却下の履歴（同じ失敗をしない）：24×24 の文字絵（2×2 黒目・ほっぺ赤み・w 口 → 食パン／ヒキガエル）、ミニマル案・お団子案・ハム太郎風案（3 案とも「ださい」）、ねこあつめ顔を正面立ち絵に乗せた版（「全然ねこあつめっぽくない」「パースがおかしい」）。グラデ・アンチエイリアスなし。**1 グリッド = 2.5 画面px で全部そろえる**（`HamuArt.PX`）：ハムスター 48×48（=120px）、グッズ 88×72（=220×180）、えさ皿 48×28（=120×70）、背景 512×256（=1280×640）。グッズ・背景のコードは旧 44×36 / 256×128 座標のまま `new Px(w*2, h*2, 2)` で 2 倍に描かれる（`data.js` の anchor もその座標系、画面px 換算は `HamuArt.ANCHOR_PX`=5）。密度がずれると安っぽく見えるので、新しい絵もこの倍率で描く。ピクセルグリッドは `Px` クラスが色ごとの `<path>` にまとめてインライン SVG（`shape-rendering: crispEdges`）で出す。画像ファイルは使わない。
...
- 2026-09-21: 絵を刷新。解像度 2 倍（1 グリッド 2.5px）、3/4 俯瞰にカメラ統一、ハムスターは楕円合成の「饅頭」に、グッズは天面が見える形に。
...
- `art.js` のハムスターは `DRAW[pose](p, c, f, tail)` に楕円で描く（48×48、右向き基準）。正面系は `blobFront()` を共用、顔は `faceFront()`。色は `c.body/belly/ear/eye/nose/line` と、種類によって null になる `c.stripe/spots/brow/face`（null なら描かない）。2 フレーム目は `f===1` のときの差分で表現。新ポーズは `DRAW`・`POSES`（anim/dur）・`HEAD`（head/brow/chin の 48 グリッド座標）を、新アクセサリーは `ACC`（文字絵、2 倍で置かれる）を追加する。
