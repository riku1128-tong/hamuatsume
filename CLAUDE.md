...
3. **イラストは「ひつじ村」風のドット絵**（2026-09-20 に SVG ベクター絵から変更。ユーザーの要望）：**ハムスターの顔は「ねこあつめ」の文法をそのまま使う**（2026-09-21 にユーザー確定。それ以前の 2×2 黒目＋ほっぺ赤み＋幅 6 の w 口、ミニマル案、お団子案、ハム太郎風案はすべて「ださい」と却下された）：目は縦 1×2 の点、鼻を頂点にした小さな ω（`ULUN` + `UULU`）、輪郭の外に 2px 突き出るヒゲ、おなか色の白いマズル（`U`）、下ぶくれの頭（col 2〜21）＋一回り小さい体＋前足。1px の暗い輪郭＋ベタ塗り＋影 1 段。グラデ・アンチエイリアスなし。**1 グリッド = 5 画面px で全部そろえる**（`HamuArt.PX`）：ハムスター 24×24（=120px）、グッズ 44×36（=220×180）、えさ皿 24×14（=120×70）、背景 256×128（=1280×640）。密度がずれると安っぽく見えるので、新しい絵もこの倍率で描く。ピクセルグリッドは `Px` クラスが色ごとの `<path>` にまとめてインライン SVG（`shape-rendering: crispEdges`）で出す。画像ファイルは使わない。
...
- 2026-09-21: ハムスター 6 ポーズをねこあつめ文法（点目・ω・ヒゲ・マズル）で描き直し。
...
- `art.js` のハムスターは文字列スプライト（`FRONT` などの定数 → `POSES[pose].f = [1枚目, 2枚目]`）。左右対称ポーズは左半分 12 列だけ書いて `M()` で鏡映、2 枚目は `F2(rows, {行番号: 差し替え行})` で差分だけ書く。文字の意味は `POSES` 直上のコメント参照（`S`/`P`/`R`/`F`/`C` は色未設定なら体色にフォールバック）。新ポーズは `POSES`（f/anim/dur）と `HEAD`（head/brow/chin の座標）を、新アクセサリーは `ACC` を追加する。行長がずれると起動時に console.warn が出る。
- グッズは `ITEM_DRAW[id](p)` に `Px` の rect/ellipse/line/tri で 44×36 に描き、最後に `outline()` で輪郭を付ける（自前で輪郭を描かない）。おへやでは `A.item(id)`（引数なし）で実寸 220×180、ショップ等では `A.item(id, w, h)` で縮小。`data.js` の `anchor` はグッズの 44×36 グリッド座標で「ハムスターの足元中央」。えさ皿の `anchor` は 24×14 グリッド座標。背景の飾りは `room()` 内の tree/bush/flower ヘルパーで置く。座標はスロット（`SLOTS`）・皿（`BOWLS`）・HUD（左 340〜720, 上 18〜210）と重ならない場所に。
