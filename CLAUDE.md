...
**2026-09-21 に Codex へ引き継ぎ。経緯・失敗の記録・次にやるべきことは [HANDOFF.md](HANDOFF.md) を先に読むこと。**
...
3. **イラストは「ひつじ村」風のドット絵**（2026-09-20 に SVG ベクター絵から変更。ユーザーの要望）：**カメラは 3/4 俯瞰で統一**（2026-09-21 に刷新）。背景の牧場が見下ろし視点なので、ハムスターもグッズも「斜め上から見た形」で描く。正面立ち絵（顔と胸が正対する猫のような二足の座り姿）は**パースが合わず却下済み**。ハムスターは**大きな丸い頭＋小さな体の 2 頭身**（体を短く描いて俯瞰に合わせる）。顔は**丸い黒ビーズの目＋白いハイライト 1px**、小さなピンクの鼻、柔らかい茶色の小さな ω（暗い体色なら明るい色）、ほっぺに薄い赤み、おなか色のマズル。**不気味になる記号は禁止**：縦スリットの目、顔の外に突き出るヒゲ、塊から生えた小さな肢、ハイライトのない黒ベタの目（これらは 2026-09-21 に「気持ち悪い」と却下された）。グッズは屋根・天面・水面が見える台形／楕円で描く。1px の細い暗い輪郭＋ベタ塗り、影・グラデなし。
   - **描き方は文字絵ではなく手続き**：`Px` の ellipse/rect/line/trap を組み合わせ、`p.part(fn, line)` で部品ごとに自動輪郭を付けて重ねる（部品の境界にも線が入る）。ハムスターは `DRAW[pose](p, c, frame, tail)`。
   - 却下の履歴（同じ失敗をしない）：24×24 の文字絵（2×2 黒目・ほっぺ赤み・w 口 → 食パン／ヒキガエル）、ミニマル案・お団子案・ハム太郎風案（3 案とも「ださい」）、ねこあつめ顔を正面立ち絵に乗せた版（「全然ねこあつめっぽくない」「パースがおかしい」）、俯瞰の饅頭にスリット目＋ヒゲ＋肢を付けた版（「気持ち悪い」＝虫に見える）。グラデ・アンチエイリアスなし。**1 グリッド = 2.5 画面px で全部そろえる**（`HamuArt.PX`）：ハムスター 48×48（=120px）、グッズ 88×72（=220×180）、えさ皿 48×28（=120×70）、背景 512×256（=1280×640）。グッズ・背景のコードは旧 44×36 / 256×128 座標のまま `new Px(w*2, h*2, 2)` で 2 倍に描かれる（`data.js` の anchor もその座標系、画面px 換算は `HamuArt.ANCHOR_PX`=5）。密度がずれると安っぽく見えるので、新しい絵もこの倍率で描く。ピクセルグリッドは `Px` クラスが色ごとの `<path>` にまとめてインライン SVG（`shape-rendering: crispEdges`）で出す。画像ファイルは使わない。
...
8. **えさ皿は 2 か所**（`D.BOWLS`、state は `bowls[2]`）。2 皿とも入っていると来訪率 ×1.15。来訪率・おみやげの基準は「入っているえさのうち attract が最大のもの」（`activeFood()`）。皿のはむは自分の皿のえさで抽選する。
9. **ハムスターはアニメーションする**：各ポーズ 2 フレーム（`POSES[pose].f`）を CSS（`.anim-alt` 交互／`.anim-blink` たまに）で切り替える。JS タイマーは使わない。おへや・図鑑・詳細・設定では `anim: true`、トースト等の小さい絵は静止。
...
- おへやに手早く住人を置く（DevTools）: `const g=hamu.game,D=HamuData;['wheel','house'].forEach((id,i)=>{g.state.inventory[id]=1;g.placeItem('s'+(i+1),id)});g.setFood('worm',1);g.freeSeats().slice(0,3).forEach((st,i)=>g.arrive(D.HAMSTERS[i],st,{arrivals:0,newHams:[]}));hamu.ui.render()`
- ブラウザが古い js をキャッシュすることがある。動きが変なら `fetch(url,{cache:'reload'})` かハードリロード。
...
- 2026-09-21: 絵を刷新。解像度 2 倍（1 グリッド 2.5px）、3/4 俯瞰にカメラ統一、グッズは天面が見える形に。ハムスターは楕円合成の 2 頭身（ビーズ目＋ハイライト、ほっぺ赤み）に確定。
- 2026-09-20: ハムスターを 24×24 に描き直して 2 フレームアニメ追加、グッズ・皿・背景のピクセル密度を 5px に統一、えさ皿を 2 か所に。セーブは v0.1 の `food` から `bowls` に自動移行（`normalize()`）。
- スマホ対応：おへやは 1280×640 のステージを縮小表示（最小 0.55 倍で横スクロール）、モバイル用えさゲージあり。
- 既知の未実装／改善候補（優先順）：
  1. アニメーションの追加（回し車が回る、グッズ側の動き、3 フレーム化など。はむ本体の 2 フレームは実装済み）
  2. 効果音・BGM（設定でオンオフ）
  3. レアはむの追加、季節イベント
  4. 図鑑のコンプリート報酬、実績
...
- `art.js` のハムスターは `DRAW[pose](p, c, f, tail)` に楕円で描く（48×48、右向き基準）。正面系は `bodyFront()` を共用、目は `eye()`（丸＋ハイライト）、口は `mouthW()`。色は `c.body/belly/ear/eye/nose/line` と、種類によって null になる `c.stripe/spots/brow/face`（null なら描かない）。2 フレーム目は `f===1` のときの差分で表現。新ポーズは `DRAW`・`POSES`（anim/dur）・`HEAD`（head/brow/chin の 48 グリッド座標）を、新アクセサリーは `ACC`（文字絵、2 倍で置かれる）を追加する。
- グッズは `ITEM_DRAW[id](p)` に `Px` の rect/ellipse/line/trap で 44×36 座標に描き（内部で 2 倍）、最後に `outline()` で輪郭を付ける（自前で輪郭を描かない）。細い線は幅 0.5 を指定すると 1 ピクセルになる。おへやでは `A.item(id)`（引数なし）で実寸 220×180、ショップ等では `A.item(id, w, h)` で縮小。`data.js` の `anchor` はグッズの 44×36 グリッド座標で「ハムスターの足元中央」。えさ皿の `anchor` は 24×14 グリッド座標。背景の飾りは `room()` 内の tree/bush/flower ヘルパーで置く。座標はスロット（`SLOTS`）・皿（`BOWLS`）・HUD（左 340〜720, 上 18〜210）と重ならない場所に。
