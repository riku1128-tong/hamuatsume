...
3. **イラストは「ひつじ村」風のドット絵**（2026-09-20 に SVG ベクター絵から変更。ユーザーの要望）：1px の暗い輪郭＋ベタ塗り＋影 1 段、2×2 の点目、ほっぺの赤み、小さな「ω」口（ねこあつめのねこの丸さ・かわいさを基準にする）。グラデ・アンチエイリアスなし。**1 グリッド = 5 画面px で全部そろえる**（`HamuArt.PX`）：ハムスター 24×24（=120px）、グッズ 44×36（=220×180）、えさ皿 24×14（=120×70）、背景 256×128（=1280×640）。密度がずれると安っぽく見えるので、新しい絵もこの倍率で描く。ピクセルグリッドは `Px` クラスが色ごとの `<path>` にまとめてインライン SVG（`shape-rendering: crispEdges`）で出す。画像ファイルは使わない。
